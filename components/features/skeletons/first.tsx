  ...
  return (
    <div className="perspective-distant rotate-z-15 -rotate-y-20 rotate-x-30 scale-[1.2] h-full w-full -translate-y-10 mask-radial-from-50% mask-r-from-50%">
      <SkeletonCard
        ...
        description="Measure openings, check foundation, review building code requirements for your property."
        badge={<Badge text="60 min" variant="success" />}
      />
      <SkeletonCard
        className="absolute bottom-8 left-8 z-20"
        icon={<IconExclamationCircle className="size-4" />}
        title="Permit Application"
        description="Submit plans to building department, coordinate inspections, ensure code compliance."
        badge={<Badge text="2-3 weeks" variant="warning" />}
      />
      <SkeletonCard
        className="absolute bottom-20 left-4 max-w-[80%] z-10"
        icon={<IconPrison className="size-4" />}
        title="Installation Planning"
        description="Schedule all trades, coordinate excavation, window install, well setup, and finishing work."
        badge={<Badge text="3-5 days" variant="success" />}
      />
    </div>
  );
};
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 flex-wrap mt-4">
        <Tag text="Building Code" />
        <Tag text="Egress Windows" />
        <Tag text="Compliance" />
      </div>
    </div>
  );
